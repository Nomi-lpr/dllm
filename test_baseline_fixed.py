...
def build_prompt_baseline(
    ...
    """
    构建baseline prompt：query始终放在最后（使用 PromptTemplate）
...
def test_baseline_fixed_icd_sequences(
    ...
    if task == "gsm8k":
        ...
        # 保存结果
        result_file = os.path.join(result_path, f"{task}_test_results_baseline_fixed.json")
